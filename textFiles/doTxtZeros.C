...
void doTxtZeros(int nparams = 5) {
  ...
  string filename = "L2residual_2023PbPb_zeros.txt";
  ...
  for (int etabin = facts[3]->GetXaxis()->GetNbins(); etabin >= 1; --etabin) {
	  ...
	  if (factors->GetXaxis()->GetBinLowEdge(etabin) < 2.9) {
	    double corr = factors->GetBinContent(factors->FindBin(higheta+0.001));
	    if (ptbin == 4 and loweta > 2.5) corr = 0;   // break;    // pT 120-170 range stops at |eta| = 2.5
	    if (ptbin == 5 and loweta > 2.0) corr = 0;   // break;    // pT 170-1000 range stops at |eta| = 1.93
	 
	    if (higheta != 0) txtfile << -loweta << " " << -higheta << " " << pts[ptbin-1] << " " << pts[ptbin] << " " << nparams << " " << pts[ptbin-1] << " " << pts[ptbin] << " " << corr << " 0 1"  << std::endl;
	    else txtfile << -loweta << " " << higheta << " " << pts[ptbin-1] << " " << pts[ptbin] << " " << nparams<< " " << pts[ptbin-1] << " " << pts[ptbin] << " " << corr << " 0 1"  << std::endl;
  ...
  for (int etabin = 1; etabin <= facts[3]->GetXaxis()->GetNbins(); ++etabin) {   // These are the narrow eta bins
	  ...
	  float higheta = facts[3]->GetXaxis()->GetBinLowEdge(etabin+1);
       ...
       if (factors->GetXaxis()->GetBinLowEdge(etabin) < 2.9) {
	 double corr = factors->GetBinContent(factors->FindBin(loweta+0.001));
	  if (ptbin == 4 and higheta > 2.5) corr = 0;    // pT 120-170 range stops at |eta| = 2.5
	  if (ptbin == 5 and higheta > 2.0) corr = 0;    // pT 170-1000 range stops at |eta| = 1.93

	  txtfile << loweta << " " << higheta << " " << pts[ptbin-1] << " " << pts[ptbin] << " " << nparams<< " " << pts[ptbin-1] << " " << pts[ptbin] << " " << corr << " 0 1"  << std::endl; 
