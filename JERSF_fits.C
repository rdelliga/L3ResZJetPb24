...
void JERSF_fits(string inFileName = "HIJEC_results/debugged_plus_JER/pbpbreco_MC_JER.root", string inFileNameDT = "HIJEC_results/debugged_plus_JER/pbpbreco_DATA_closureandJER.root") {
  ...
  map<int, TH3D*> asymmMCs, asymmDTs; // _a10 to a55?
  ...
  for (int i = 0; i < nalphas; ++i) {
    ...
    asymmDTs[i] = (TH3D*)inFileDT->Get(Form("hibin_-1.0_0.0/eta_-5.2_5.2/asymmdist3D_%d",alphabins[i]));
  }
  ...
  for (int ptbin = 3; ptbin <= asymmDT->GetXaxis()->GetNbins(); ++ptbin) {
	...
	c1->SetLogy();
	c1->Print(Form("jersfhists_L2/asymm_DT_ptbin_%d_etabin_%d_a%d.pdf",ptbin,etabin,alphabin));
      }
      }
