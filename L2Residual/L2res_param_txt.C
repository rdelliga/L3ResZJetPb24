...
void L2res_param_txt(TString inFileName = "ptfits/testing_pt_dep.root", TString kfactFile = "L2fits_AK4CHSjetveto_all/correctionfile.root", TString outFileName = "L2residual-param.txt", bool loglin = true) {
   ...
   string header = "{1 JetEta 1 JetPt ([0]+[1]*log(x))*[2] Correction L2Relative}";
   ...
   for (int etabin = kfacts->GetXaxis()->GetNbins(); etabin >= 1; --etabin) {
     ...
     if (kfacts->GetXaxis()->GetBinLowEdge(etabin) < 2.9) {
       ...
       if (higheta != 0) txtfile << -loweta << " " << -higheta << " " << nparams << " " <<  ptmin << " " << ptmax << " " << params->GetParameter(0) << " " << (loglin ? params->GetParameter(1) : 0) <<  " " << kfacts->GetBinContent(etabin) << std::endl;
       else txtfile << -loweta << " " << higheta << " "  << nparams<< " "  << " " <<  ptmin << " " << ptmax << " " << params->GetParameter(0) << " " << (loglin ? params->GetParameter(1) : 0) <<  " " << kfacts->GetBinContent(etabin) << std::endl;
  ...
  for (int etabin = 1; etabin <= kfacts->GetXaxis()->GetNbins(); ++etabin) {
    ...
    if (kfacts->GetXaxis()->GetBinLowEdge(etabin) < 2.9) {
      txtfile << loweta << " " << higheta << " "  << nparams<< " "  << " " <<  ptmin << " " << ptmax << " " << params->GetParameter(0) << " " << (loglin ? params->GetParameter(1) : 0) <<  " " << kfacts->GetBinContent(etabin) << std::endl;
    }
  }
