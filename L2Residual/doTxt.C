...
void doTxt(TString inFileName = "../RERECORESULTS/kfactor_rerunall_combined_allpts.root", TString outFileName = "L2residual.txt" ) {
  ...
  for (int ptbin = 1; ptbin <= histograms::nptforjec; ++ptbin) {
    ...
    TH1D* h = dynamic_cast<TH1D*>(inFile->Get(Form("corrections_%s", lbl.c_str())));
    if (!h) continue;
    corrBins.push_back({histograms::ptforjec[ptbin - 1], histograms::ptforjec[ptbin], h});
  }

  if (corrBins.empty()) {
    std::cerr << "No corrections_* histograms found in " << inFileName << std::endl;
    txtfile.close();
    return;
  }

  std::sort(corrBins.begin(), corrBins.end(), [](const CorrBin& a, const CorrBin& b) {
    return a.lowPt < b.lowPt;
  });
  ...
  facts[4] = (TH1D*)inFile2->Get(Form("corrs_%dto%d",pts[4],pts[5])); */
    ...
    for (int etabin = nEtaBins; etabin >= 1; --etabin) {
	  ...
	  if (factors->GetXaxis()->GetBinLowEdge(etabin) < 2.9) {
	    float lowpt = 0.001;
      if (cb.lowPt > 25) lowpt = cb.lowPt;
	 
      if (higheta != 0) txtfile << -loweta << " " << -higheta << " " << lowpt << " " << cb.highPt << " " << nparams << " " << cb.lowPt << " " << cb.highPt << " " << factors->GetBinContent(factors->FindBin(higheta+0.001)) << " 0 1"  << std::endl;
	    
      else txtfile << -loweta << " " << higheta << " " << lowpt << " " << cb.highPt << " " << nparams<< " " << cb.lowPt << " " << cb.highPt << " " << factors->GetBinContent(factors->FindBin(higheta+0.001)) << " 0 1"  << std::endl;
  ...
  for (int etabin = 1; etabin <= nEtaBins; ++etabin) {   // These are the narrow eta bins
    ...
    txtfile << loweta << " " << higheta << " " << lowpt << " " << cb.highPt << " " << nparams<< " " << cb.lowPt << " " << cb.highPt << " " << factors->GetBinContent(factors->FindBin(loweta+0.001)) << " 0 1"  << std::endl; 
